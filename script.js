...
//---------------------------------------------------------------------
// Immediately populate the table & the selection
//---------------------------------------------------------------------
(function() {
    ...
    function populateSelection(players) {
        ...
        // populating the select elem
        const selectMenu = document.getElementById("team-filter");
        teams.forEach((team) => {
...
inputSearchElem.addEventListener("keyup", () => {
    let word = inputSearchElem.value.toLowerCase();
...
function doFilter(rows, column, word) {
    ...
    // loop through all table rows and hide rows that 
    // have cells that dont match the search query
    for (let i = 0; i < rows.length; i++) {
        let td = rows[i].getElementsByTagName("td")[column];
        let txtValue = td.innerHTML.toLowerCase();
...
for (header of headers) {
    header.addEventListener("click", (event) => {
        let colIndex = event.srcElement.cellIndex;
        let rows = document.querySelectorAll("tbody tr");
        let rowsArray = [];
